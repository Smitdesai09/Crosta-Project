...
const SkeletonBlock = ({ className = "" }) => (
  <div aria-hidden="true" className={`animate-pulse rounded-lg bg-neutral-100 ${className}`} />
);
    ...
    <div className="mt-5 grid gap-2">
      {Array.from({ length: PRODUCT_SKELETON_VARIANT_COUNT }, (_, index) => (
        <div
          ...
          className="flex items-center justify-between rounded-xl border border-neutral-200 bg-neutral-100 px-4 py-3"
        >
          <div>
...
const ProductFormModal = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4 backdrop-blur-sm">
      <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-xl border border-neutral-200 bg-white shadow-xl">
        <div className="flex items-start justify-between border-b border-neutral-200 px-5 py-4">
          ...
          <button
            type="button"
            onClick={onClose}
            className="cursor-pointer rounded-lg p-2 text-neutral-400 transition-all duration-200 hover:bg-neutral-100 hover:text-neutral-900"
          >
            <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" />
            </svg>
        ...
        <form onSubmit={onSubmit} className="p-5">
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              <label className="mb-1.5 block text-sm font-medium text-neutral-900">Product Name</label>
              <input
                type="text"
                value={form.name}
            ...
            <div>
              ...
              <input
                ...
                className={`w-full rounded-lg border bg-neutral-50 px-3 py-2.5 text-sm text-neutral-900 outline-none transition-colors placeholder:text-neutral-400 focus:border-[#ff6d33] focus:ring-2 focus:ring-[#ff6d33]/30 ${errors.category ? "border-red-400" : "border-neutral-200"
                  }`}
              />
          ...
          <div className="mt-4 rounded-xl border border-neutral-200 bg-neutral-50 p-4">
            <div className="mb-3 flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-neutral-500">Add at least one size or variant with a price.</p>
              </div>
            ...
            <div className="grid gap-3">
              {form.variants.map((variant, index) => (
                <div
                  key={`variant-${index}`}
                  className="grid gap-3 rounded-xl border border-neutral-200 bg-white p-3 md:grid-cols-[1fr_180px_auto]"
                >
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full rounded-lg border bg-neutral-50 px-3 py-2.5 text-sm text-neutral-900 outline-none transition-colors placeholder:text-neutral-400 focus:border-[#ff6d33] focus:ring-2 focus:ring-[#ff6d33]/30 ${errors.variants[index]?.name ? "border-red-400" : "border-neutral-200"
                        }`}
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="0.00"
                      className={`w-full rounded-lg border bg-neutral-50 px-3 py-2.5 text-sm text-neutral-900 outline-none transition-colors placeholder:text-neutral-400 focus:border-[#ff6d33] focus:ring-2 focus:ring-[#ff6d33]/30 ${errors.variants[index]?.price ? "border-red-400" : "border-neutral-200"
                        }`}
                    />
          ...
          <label className="mt-4 flex items-center gap-3 rounded-xl border border-neutral-200 bg-neutral-50 px-4 py-3">
            <input
              type="checkbox"
              checked={form.isAvailable}
              onChange={(event) => onChange("isAvailable", event.target.checked)}
              className="h-4 w-4 rounded border-neutral-300 text-[#ff6d33] focus:ring-[#ff6d33]/30"
            />
            <div>
              <p className="text-sm font-medium text-neutral-900">Available for sale</p>
            </div>
          </label>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-xl border border-neutral-200 bg-white shadow-xl">
        ...
        <div className="px-5 py-4">
          <div
            className={`rounded-xl border p-4 ${isUnavailable ? "border-[#E0F2F1] bg-[#E0F2F1]" : "border-red-100 bg-red-50"
              }`}
          >
            <p className="text-sm font-medium text-neutral-900">{product.name}</p>
            <p className="mt-1 text-sm text-neutral-500">{formatCategory(product.category)}</p>
          </div>
        </div>
    ...
    <div className="h-full w-full flex flex-col gap-6 overflow-y-auto p-4 lg:p-6">
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex flex-wrap gap-3">
          {isAdmin ? (
            <Button
              type="button"
              className={`inline-flex items-center gap-2 self-start px-5 py-3 bg-[#ff6d33] hover:bg-orange-700 focus:ring-[#ff6d33]/50 text-white shadow-sm ${PRODUCT_ACTION_BUTTON_CLASS}`}
          ...
          PRODUCT_STATS.map((card) => (
            <Card key={card.key} className={`min-h-[112px] ${PRODUCT_CARD_HOVER_CLASS}`}>
              <div className="flex items-center gap-4">
                ...
                <p className="ml-auto text-4xl font-bold leading-none text-neutral-900">{productStats[card.key]}</p>
              </div>
            </Card>
          ))
        )}
          ...
          <div className="relative w-full md:col-span-7">
            ...
            {searchTerm ? (
              <button
                ...
                title="Clear search"
              >
                <svg className="h-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
            ) : null}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 xl:grid-cols-3">
        {isPageLoading ? (
          ...
          <div className="col-span-full flex flex-col items-center justify-center rounded-xl border border-neutral-200 bg-white py-16 text-neutral-400 shadow-sm">
          ...
          filteredProducts.map((product) => {
            ...
            return (
              <Card key={product._id} className={PRODUCT_CARD_HOVER_CLASS}>
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-2">
                      <div className="flex items-center gap-2 text-xs font-semibold text-neutral-500">
                        <span className="inline-flex rounded-full bg-neutral-100 px-2.5 py-1 text-neutral-600">
                  ...
                  {isAdmin ? (
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
                        className={`border-neutral-200 text-neutral-500 hover:bg-neutral-100 hover:text-neutral-900 ${PRODUCT_ICON_BUTTON_CLASS}`}
                ...
                {/* FIX: Grey default, Orange strictly on hover */}
                <div className="mt-5 grid gap-2">
                  {sortedVariants.map((variant) => (
                    <div
                      key={`${product._id}-${variant.name}`}
                      className="flex items-center justify-between rounded-xl border border-neutral-200 bg-neutral-50 px-4 py-3 transition-colors hover:border-[#ff6d33]/30 hover:bg-[#fff5f1]"
                    >
                      <div>
                        <p className="text-sm font-semibold text-neutral-900">{variant.name}</p>
                        <p className="text-xs text-neutral-500">Variant option</p>
                      </div>
                      <span className="text-sm font-bold text-neutral-900">{formatCurrency(variant.price)}</span>
                    </div>
                  ))}
                </div>
              </Card>
