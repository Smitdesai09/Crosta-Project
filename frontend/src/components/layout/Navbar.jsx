...
const Navbar = ({ onMenuClick, isSidebarCollapsed, setIsSidebarCollapsed, isMobileOpen, setIsMobileOpen }) => {
  return (
    <header className="sticky top-0 z-30 h-14 bg-surface-white border-b border-border-main flex items-center justify-between px-4 lg:px-6">
      <div className="flex items-center gap-3">
        ...
        <button 
          ...
          className={`hidden lg:flex items-center justify-center p-2 rounded-lg transition-colors duration-200 ${
            isSidebarCollapsed ? 'bg-surface-gray text-text-primary' : 'text-text-secondary hover:bg-surface-gray hover:text-text-primary'
          }`}
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h16" /></svg>
        </button>
