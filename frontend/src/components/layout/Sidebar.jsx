...
const Sidebar = ({ isCollapsed, isMobileOpen, setIsMobileOpen }) => {
  const menuItems = [
    ...
    ...(PERMISSIONS.VIEW_ANALYTICS ? [{ icon: Icons.analytics, label: "Analytics", path: "/analytics" }] : []),
    ...(PERMISSIONS.VIEW_ADMIN_PANEL ? [{ icon: Icons.admin, label: "Admin Panel", path: "/admin-panel" }] : []),
  ];

  const sidebarContent = (
    <div className="flex flex-col h-full bg-neutral-deep">
      {/* Logo Area - Increased height to h-20 and added vertical padding */}
      <div className={`flex items-center h-20 px-3 py-4 border-b border-white/10 ${isCollapsed ? 'justify-center' : 'justify-start'}`}>
        <img 
          src={brandLogo} 
          alt="Crosta Logo" 
          // Increased expanded width, and made collapsed bigger to stand out
